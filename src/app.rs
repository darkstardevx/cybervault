...
impl App {
    ...
    /// Overwrites the in-progress secret with one generated by Keysmith.
    /// Only meaningful during `AddSecret` — the generated value lands in
    /// `input_buffer` exactly like manually-typed input, so it's still
    /// editable and `confirm_secret` needs no changes to accept it.
    pub fn generate_secret(&mut self, kind: crate::keysmith_gen::Kind) {
        ...
        match crate::keysmith_gen::generate(kind) {
            ...
            Err(e) => self.status = Some(format!("keysmith: {e}")),
        }
    }
...
mod tests {
    ...
    #[test]
    fn generate_secret_populates_input_buffer_from_real_keysmith() {
        ...
        app.generate_secret(crate::keysmith_gen::Kind::Password);

        assert_eq!(app.input_buffer.chars().count(), 24, "keysmith password --raw --length 24 should be exactly 24 chars");
        assert!(app.status.as_deref().unwrap_or_default().contains("generated via keysmith"));
        std::fs::remove_file(&path).ok();
    }

    #[test]
    fn generate_secret_ignored_outside_add_secret_mode() {
